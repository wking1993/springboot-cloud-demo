...
@EnableConfigurationProperties(value = SwaggerListProperties.class)
...
public class SwaggerAutoConfiguration extends WebMvcConfigurerAdapter {
		...
		for (SwaggerProperties conf : list) {
			...
			if (conf.isAuthHeader()) {
				parameterBuilder.parameterType("header") //参数类型支持header、cookie、body、query etc
					...
					.modelRef(new ModelRef("string")) //制定参数的类型
					.required(false).build(); //非必须，这里是全局配置，然而在登录时候，是不用验证的
				parameters.add(parameterBuilder.build());
			}
			index++;
			Docket doc = new Docket(DocumentationType.SWAGGER_2)
							.groupName(conf.getGroup())
							.apiInfo(apiInfo(conf))
	...
	private ApiInfo apiInfo(SwaggerProperties conf) {
		return new ApiInfoBuilder().title(conf.getTitle())
